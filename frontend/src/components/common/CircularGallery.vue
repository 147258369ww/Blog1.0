...
class App {
  ...
  centerGallery() {
    // 计算中心位置：总项目数的一半减去可见项目的一半
    if (this.medias && this.medias.length > 0 && this.medias[0]) {
      const centerIndex = Math.floor(this.medias.length / 4);
      const centerPosition = this.medias[0].width * centerIndex;
