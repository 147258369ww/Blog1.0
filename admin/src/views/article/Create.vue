...
import { articleApi, type ArticleCreateRequest, type ArticleStatus } from '@/api/article'
...
// 表单数据
const formData = reactive<ArticleCreateRequest>({
  title: '',
  content: '',
  coverImage: '',
...
const saveArticle = async (status: ArticleStatus) => {
  ...
  try {
    ...
    if (response.success) {
      ElMessage.success(status === 'draft' ? '草稿保存成功' : '文章发布成功')
 ...
 */
const handleCancel = () => {
  ElMessageBox.confirm('确定要取消吗？未保存的内容将会丢失。', '确认取消', {
